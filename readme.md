...
| 操作 | 说明 |
|------|------|
| 打开文件 | 单击文件名；有标记时跳到第一个标记位置（左下角提示可在设置「显示」中配置） |
| 打开标记 | 展开「游标 / 函数 / 匹配」后单击某一条；左下角按显示配置提示「类型：名称」 |
| 从分组中移除 | 右键文件 → **从分组中移除** |
| 添加游标 / 添加函数 / 字符匹配 | 右键文件；展开文件后先看到「游标 / 函数 / 匹配」分组，再展开查看具体标记 |
| 删除 / 重命名标记 | 展开到具体标记后右键 |
| 复制路径 | 右键文件 → **复制路径** |
...
侧边栏标题栏点击 **齿轮图标** 可打开「Tab Groups 设置」：

- **通用**：打开 `.vscode/tab-groups.json`；**配置版本更新**（配置 schema 落后时升级写回）
- **显示**：**标记左下角显示**（默认一直显示；可选「显示秒数」或「关闭」；改完自动保存）
- **快捷键**：点击快捷键框后按下组合键录入，保存后写入 `.vscode/settings.json` 并同步到本机 keybindings

跳转到标记时，左下角会按「显示」里的配置提示当前标记名称。
...
| 添加游标 | `Ctrl+Shift+L` | 编辑器内；追加游标标记 |
| 添加函数 | `Ctrl+Shift+;` | 编辑器内；记录当前所在函数并跳转优先按符号名 |
| 字符匹配 | `Ctrl+Shift+'` | 编辑器内；用选区/词做模糊定位标记 |
| 上一标记 | `Ctrl+Shift+[` | 在当前文件标记间循环；左下角提示见设置「显示」 |
| 下一标记 | `Ctrl+Shift+]` | 在当前文件标记间循环；左下角提示见设置「显示」 |

> **提示**：`Ctrl+Shift+P` 与 VS Code 命令面板默认快捷键相同，若冲突请在设置页「快捷键」分类中改绑删除分组快捷键。

保存快捷键需要已打开**单根工作区**；无工作区时可打开设置页预览，但无法保存。
...
仓库里的 `example/` 文件夹有一份对照示例（分组数据、工作区设置、本机快捷键），字段说明见其中的 `explain.md`。
